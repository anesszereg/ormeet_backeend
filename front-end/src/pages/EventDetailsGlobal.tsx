...
const EventDetailsGlobal = () => {
    ...
    const fetchEventData = async () => {
      ...
      try {
        ...
        const organizerEventsCount = event.organizerId
          ? allEvents.filter(
              (e: ApiEvent) => e.organizerId === event.organizerId,
            ).length
          : 0;
        ...
        const availableSpots =
          event.ticketTypes?.reduce(
            (sum, tt) =>
              sum +
              (Number(tt.quantityTotal || 0) - Number(tt.quantitySold || 0)),
            0,
          ) ?? 0;
        ...
        if (user) {
          try {
            const [favoriteStatus, followStatus] = await Promise.all([
              ...
              event.organizerId
                ? userPreferencesService.isFollowingOrganizer(event.organizerId)
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      if (isFavorite) {
        ...
        setIsFavorite(false);
        setEventData((prev) =>
          prev ? { ...prev, favorites: Math.max(0, prev.favorites - 1) } : prev,
        );
        ...
        setIsFavorite(true);
        setEventData((prev) =>
          prev ? { ...prev, favorites: prev.favorites + 1 } : prev,
        );
      ...
      alert(
        err.response?.data?.message || t("eventDetails.alerts.favoriteFailed"),
      );
    } finally {
  ...
  const handleToggleFollow = async () => {
    ...
    try {
      ...
      alert(
        err.response?.data?.message || t("eventDetails.alerts.followFailed"),
      );
    } finally {
  ...
  const handleNewsletterSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      ...
      setNewsletterMessage(t("eventDetails.newsletter.invalidEmail"));
      return;
    }
    ...
    try {
      ...
      setNewsletterStatus("error");
      setNewsletterMessage(
        err.response?.data?.message || t("eventDetails.newsletter.error"),
      );
    } finally {
      ...
      <div className="flex flex-col min-h-screen w-full bg-white">
        ...
        <div className="flex flex-col items-center justify-center flex-1 py-20">
          <p className="text-red-500 mb-4">
            {error || t("eventDetails.error.notFound")}
          </p>
          <button
            onClick={handleGoBack}
            className="text-[#FF4000] font-semibold hover:underline"
          >
            {t("eventDetails.error.goBack")}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col min-h-screen w-full bg-white pb-20 lg:pb-0">
          ...
          <div className="relative w-full aspect-[2.5/1] md:aspect-[2.8/1] lg:aspect-[2.2/1] rounded-2xl overflow-hidden">
            ...
            >
              <img
                src={NextImageIcon}
                alt="Previous"
                className="w-full h-full scale-x-[-1] rtl:scale-x-[1]"
              />
            </button>
            ...
            <div className="absolute bottom-4 start-1/2 -translate-x-1/2 flex items-center gap-2">
              {images.map((_, index) => (
                <button
                  ...
                  aria-label={t("eventDetails.actions.goToImage", {
                    number: index + 1,
                  })}
                />
              ))}
        ...
        <div className="w-full max-w-[1200px] mx-auto">
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-6 lg:gap-12">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center flex-wrap gap-3 md:gap-4 mb-8">
                ...
                >
                  ...
                  <span className="text-sm font-semibold text-black">
                    {eventData.rating.toFixed(1)}
                  </span>
                  <span className="text-sm text-[#757575] underline decoration-dotted underline-offset-2">
                    • {eventData.reviewCount} {t("eventDetails.stats.reviews")}
                  </span>
                </button>
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  {eventData.availableSpots > 0 ? (
                    <span
                      className={`text-sm ${eventData.availableSpots <= 10 ? "text-orange-500 font-medium" : "text-[#757575]"}`}
                    >
                      {eventData.availableSpots.toLocaleString()}{" "}
                      {t(
                        "eventDetails.stats.spotsAvailable",
                        "spots available",
                      )}
                    </span>
                  ) : (
                    <span className="text-sm text-red-500 font-medium">
                      {t("eventDetails.stats.soldOut", "Sold out")}
                    </span>
                  )}
                </div>
                ...
                <button
                  ...
                  title={
                    !user
                      ? t("eventDetails.actions.loginToFavorite")
                      : isFavorite
                        ? t("eventDetails.actions.removeFromFavorites")
                        : t("eventDetails.actions.addToFavorites")
                  }
                >
                  <svg
                    className="w-5 h-5 text-[#FF4000]"
                  ...
                  <span className="text-sm text-[#757575]">
                    {eventData.favorites.toLocaleString()}{" "}
                    {t("eventDetails.stats.favorites")}
                  </span>
                </button>
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    {isFavorite ? (
                      <svg
                        className="w-[42px] h-[42px] text-[#FF4000]"
                        fill="currentColor"
                        viewBox="0 0 24 24"
                      >
                        <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
              ...
              <div className="mb-6">
                <h2 className="text-base font-bold text-black mb-2">
                  {t("eventDetails.dateTime.sectionTitle")}
                </h2>
              ...
              <div className="mb-6">
                <h2 className="text-base font-bold text-black mb-2">
                  {t("eventDetails.location.sectionTitle")}
                </h2>
                <p className="text-sm text-black">
                  {eventData.venue}{" "}
                  <span className="text-[#757575] mx-2">|</span>
                ...
                >
                  {showLocationSection
                    ? t("eventDetails.location.closeMap")
                    : t("eventDetails.location.seeOnMap")}
                </button>
                    ...
                    <div className="relative w-full h-[400px] lg:h-[500px]">
                      ...
                      <div className="absolute top-4 end-4 flex flex-col gap-2">
                        ...
                        <button className="w-10 h-10 bg-white rounded-lg shadow-md flex items-center justify-center hover:bg-[#F8F8F8] transition-colors">
                          ...
                          >
                            <path
                              d="M2.5 7.5V2.5H7.5M12.5 2.5H17.5V7.5M17.5 12.5V17.5H12.5M7.5 17.5H2.5V12.5"
                              stroke="#4F4F4F"
                              strokeWidth="1.5"
                              strokeLinecap="round"
                              strokeLinejoin="round"
                        ...
                        <button className="w-10 h-10 bg-white rounded-lg shadow-md flex items-center justify-center hover:bg-[#F8F8F8] transition-colors">
                          ...
                          >
                            ...
                            <path
                              d="M10 2V5M10 15V18M18 10H15M5 10H2"
                              stroke="#4F4F4F"
                              strokeWidth="1.5"
                              strokeLinecap="round"
                            />
              ...
              <div className="mb-6">
                <h2 className="text-lg font-bold text-black mb-6">
                  {t("eventDetails.guidelines.sectionTitle")}
                </h2>
                  ...
                  <div className="flex gap-3 mb-5">
                    ...
                    <div>
                      <h3 className="text-sm font-semibold text-black mb-1">
                        {t("eventDetails.guidelines.ageRequirement.label")}
                      </h3>
                      <p className="text-sm text-[#4F4F4F]">
                        {eventData.guidelines?.ageRequirement ||
                          (eventData.ageLimit
                            ? t(
                                "eventDetails.guidelines.ageRequirement.defaultText",
                                { age: eventData.ageLimit },
                              )
                            : t(
                                "eventDetails.guidelines.ageRequirement.noRestriction",
                              ))}
                      </p>
                    </div>
                  </div>
                ...
                {eventData.guidelines?.entryPolicy && (
                  <div className="flex gap-3 mb-5">
                    ...
                    <div>
                      <h3 className="text-sm font-semibold text-black mb-1">
                        {t("eventDetails.guidelines.entryPolicy.label")}
                      </h3>
                      <p className="text-sm text-[#4F4F4F]">
                        {eventData.guidelines.entryPolicy}
                        {eventData.allowReentry !== undefined && (
                          <span>
                            {" "}
                            {eventData.allowReentry
                              ? t(
                                  "eventDetails.guidelines.entryPolicy.reentryAllowed",
                                )
                              : t(
                                  "eventDetails.guidelines.entryPolicy.noReentry",
                                )}
                          </span>
                        )}
                      </p>
                ...
                {eventData.guidelines?.accessibleInfo && (
                  <div className="flex gap-3 mb-5">
                    ...
                    <div>
                      <h3 className="text-sm font-semibold text-black mb-1">
                        {t("eventDetails.guidelines.accessibility.label")}
                      </h3>
                      <p className="text-sm text-[#4F4F4F]">
                        {eventData.guidelines.accessibleInfo}
                      </p>
                ...
                {/* Refund Policy */}
                {(eventData.refundsAllowed !== undefined ||
                  ...
                  <div className="flex gap-3 mb-5">
                    ...
                    <div>
                      <h3 className="text-sm font-semibold text-black mb-1">
                        {t("eventDetails.guidelines.refundPolicy.label")}
                      </h3>
                      <p className="text-sm text-[#4F4F4F]">
                        {eventData.guidelines?.refundPolicy ||
                          (eventData.refundsAllowed
                            ? t(
                                "eventDetails.guidelines.refundPolicy.refundsAvailable",
                              )
                            : t(
                                "eventDetails.guidelines.refundPolicy.noRefunds",
                              ))}
                      </p>
                    </div>
                  </div>
                ...
                {/* Prohibited Items */}
                {eventData.guidelines?.prohibitedItems &&
                  eventData.guidelines.prohibitedItems.length > 0 && (
                    <div className="flex gap-3 mb-5">
                      <img
                        src={ProhibitedItemsIcon}
                        alt="Prohibited items"
                        className="w-6 h-6 shrink-0"
                      />
                      <div>
                        <h3 className="text-sm font-semibold text-black mb-2">
                          {t("eventDetails.guidelines.prohibitedItems.label")}
                        </h3>
                        <ul className="space-y-2">
                          {eventData.guidelines.prohibitedItems.map(
                            (item, index) => (
                              <li
                                key={index}
                ...
                {/* Allowed Items */}
                {eventData.guidelines?.allowedItems &&
                  eventData.guidelines.allowedItems.length > 0 && (
                    <div className="flex gap-3">
                      <img
                        src={AllowedItemsIcon}
                        alt="Allowed items"
                        className="w-6 h-6 shrink-0"
                      />
                      <div>
                        <h3 className="text-sm font-semibold text-black mb-2">
                          {t("eventDetails.guidelines.allowedItems.label")}
                        </h3>
                        <ul className="space-y-2">
                          {eventData.guidelines.allowedItems.map(
                            (item, index) => (
                              <li
                                key={index}
                                className="flex items-start gap-2"
              ...
              <div className="mb-8">
                <h2 className="text-lg font-bold text-black mb-4">
                  {t("eventDetails.organizer.sectionTitle")}{" "}
                  <span className="font-bold">
                    {t("eventDetails.organizer.sectionTitleBold")}
                  </span>
                </h2>
                ...
                <div className="bg-[#F0F8F7] rounded-2xl p-6">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 mb-4">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="relative">
                        ...
                        <div className="absolute -bottom-1 -end-1 w-6 h-6 bg-[#FF4000] rounded-full flex items-center justify-center">
                          ...
                          >
                            <path
                              d="M4.66675 7.00004L6.41675 8.75004L9.91675 5.25004"
                              stroke="white"
                              strokeWidth="1.5"
                              strokeLinecap="round"
                              strokeLinejoin="round"
                      ...
                      <div>
                        <h3 className="text-base font-semibold text-black mb-3">
                          {eventData.organizerName || "Organizer"}
                        </h3>
                        ...
                        <div className="flex items-center gap-4">
                          <div className="flex items-baseline gap-1">
                            <span className="text-sm font-bold text-black">
                              {eventData.organizerFollowers.toLocaleString()}
                            </span>
                            <span className="text-xs text-[#757575]">
                              {t(
                                "eventDetails.organizer.followers",
                                "Followers",
                              )}
                            </span>
                          </div>
                          ...
                          <div className="flex items-baseline gap-1">
                            <span className="text-sm font-bold text-black">
                              {eventData.organizerEventsCount.toLocaleString()}
                            </span>
                            <span className="text-xs text-[#757575]">
                              {t("eventDetails.organizer.events", "Events")}
                            </span>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        title={
                          !user
                            ? t("eventDetails.organizer.loginToFollow")
                            : isFollowing
                              ? t("eventDetails.organizer.following")
                              : t("eventDetails.organizer.follow")
                        }
                      >
                        {isFollowing
                          ? t("eventDetails.organizer.following")
                  ...
                  {eventData.organizerName && (
                    <p className="text-sm text-[#4F4F4F] mb-4 leading-relaxed">
                      {t("eventDetails.organizer.isOrganizing", {
                  ...
                  {(eventData.organizerWebsite ||
                    ...
                    Object.keys(eventData.organizerSocials).length > 0) && (
                    <div className="flex flex-wrap items-center gap-3 pt-4 border-t border-[#E0EBEA]">
                      {eventData.organizerWebsite && (
                        <a
                          href={
                            eventData.organizerWebsite.startsWith("http")
                              ? eventData.organizerWebsite
                              : `https://${eventData.organizerWebsite}`
                          }
                          target="_blank"
                      ...
                      {Object.entries(eventData.organizerSocials).map(
                        ([platform, url]) => (
                          <a
                            key={platform}
                            href={
              ...
              >
                <div className="flex items-center gap-2 mb-6">
                  <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                    <path
                      d="M10 1.66699L12.575 6.88366L18.3333 7.72533L14.1667 11.7837L15.15 17.517L10 14.8087L4.85 17.517L5.83333 11.7837L1.66667 7.72533L7.425 6.88366L10 1.66699Z"
                      fill="#FFA500"
                      stroke="#FFA500"
                      strokeWidth="1.5"
                      strokeLinecap="round"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  {allReviews.length === 0 && (
                    <p className="text-sm text-[#757575] col-span-2">
                      {t("eventDetails.reviews.noReviews")}
                    </p>
                  )}
                </div>
              ...
              <div className="mt-12 mb-8">
                <h2 className="text-xl text-black text-center mb-8">
                  {t("eventDetails.faq.sectionTitle")}{" "}
                  <span className="font-bold">
                    {t("eventDetails.faq.sectionTitleBold1")}
                  </span>{" "}
                  {t("eventDetails.faq.sectionTitleMid")}{" "}
                  <span className="font-bold">
                    {t("eventDetails.faq.sectionTitleBold2")}
                  </span>
                </h2>
                ...
                <div className="space-y-3">
                  ...
                  <div className="bg-[#F8F8F8] rounded-lg overflow-hidden">
                    ...
                    >
                      <span className="text-sm font-semibold text-black">
                        {t("eventDetails.faq.items.0.question")}
                      </span>
                      <svg
                        width="16"
                        height="16"
                  ...
                  <div className="bg-[#F8F8F8] rounded-lg overflow-hidden">
                    ...
                    >
                      <span className="text-sm font-semibold text-black">
                        {t("eventDetails.faq.items.1.question")}
                      </span>
                      <svg
                        width="16"
                        height="16"
                  ...
                  <div className="bg-[#F8F8F8] rounded-lg overflow-hidden">
                    ...
                    >
                      <span className="text-sm font-semibold text-black">
                        {t("eventDetails.faq.items.2.question")}
                      </span>
                      <svg
                        width="16"
                        height="16"
                  ...
                  <div className="bg-[#F8F8F8] rounded-lg overflow-hidden">
                    ...
                    >
                      <span className="text-sm font-semibold text-black">
                        {t("eventDetails.faq.items.3.question")}
                      </span>
                      <svg
                        width="16"
                        height="16"
                  ...
                  <div className="bg-[#F8F8F8] rounded-lg overflow-hidden">
                    ...
                    >
                      <span className="text-sm font-semibold text-black">
                        {t("eventDetails.faq.items.4.question")}
                      </span>
                      <svg
                        width="16"
                        height="16"
              ...
              {moreFromOrganizerEvents.length > 0 && (
                <div className="mt-12 mb-8">
                  <h2 className="text-xl text-black mb-6">
                    {t("eventDetails.moreFromOrganizer")}{" "}
                    <span className="font-bold">
                      {eventData.organizerName ||
                        t("eventDetails.moreFromOrganizerFallback")}
                    </span>
                  </h2>
            ...
            <div className="lg:w-[280px] xl:w-[320px] flex flex-col items-start lg:items-end gap-4 lg:sticky lg:top-6 lg:self-start">
              ...
              {(() => {
                const now = new Date();
                const isPast = eventData.endAtRaw
                  ? new Date(eventData.endAtRaw) < now
                  : false;
                const isSoldOut =
                  eventData.availableSpots === 0 &&
                  (eventData.capacity ?? 0) > 0;
                const isDisabled = isPast || isSoldOut;
                return (
                  <button
                    onClick={() =>
                      !isDisabled && navigate(`/event/${eventId}/tickets`)
                    }
                    disabled={isDisabled}
          ...
          <div className="mt-16 mb-12">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl text-black">
                <span className="font-bold">
                  {t("eventDetails.trending.sectionTitle")}
                </span>{" "}
                {t("eventDetails.trending.sectionTitleSuffix")}
              </h2>
              <div className="flex items-center gap-3">
                <span className="text-sm text-[#757575]">
                  {trendingPage} {t("eventDetails.trending.paginationOf")}{" "}
                  {totalPages}
                </span>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
              ...
              <div className="flex-1">
                <h2 className="text-xl sm:text-2xl lg:text-3xl text-black mb-3">
                  <span className="font-bold">
                    {t("eventDetails.newsletter.title")}
                  </span>{" "}
                  {t("eventDetails.newsletter.titleSuffix")}
                </h2>
              ...
              <div className="flex-1 max-w-xl w-full">
                <form
                  ...
                  className="relative flex items-center bg-white border border-[#EEEEEE] rounded-full p-1 h-12"
                >
                  <input
                    type="email"
                  ...
                  <button
                    ...
                    className="shrink-0 inline-flex items-center gap-1.5 px-5 h-10 bg-[#FF4000] text-white font-semibold hover:bg-[#E63900] disabled:opacity-60 disabled:cursor-not-allowed transition-colors text-sm whitespace-nowrap rounded-full"
                  >
                    {isSubmittingNewsletter ? (
                      <>
                        ...
                        <span>{t("eventDetails.newsletter.submitting")}</span>
                      </>
                    ) : (
                      <>
                ...
                {newsletterStatus === "success" && (
                  <p className="mt-2 text-xs text-green-600">
                    {newsletterMessage ||
                      t(
                        "eventDetails.newsletter.success",
                        "You are subscribed!",
                      )}
                  </p>
                )}
      ...
      <div
        className="lg:hidden fixed bottom-0 inset-x-0 z-40 bg-white border-t border-[#EEEEEE] px-4 py-3 flex items-center justify-between gap-3"
